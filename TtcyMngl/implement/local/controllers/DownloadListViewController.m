...
-(void)createDataTable
{
    ...
    _dataTable.contentOffset = CGPointMake(0, 0);
    [_dataTable setTranslatesAutoresizingMaskIntoConstraints:NO];
    [self.view addSubview:_dataTable];
    
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[_dataTable]-0-|" options:0 metrics:0 views:NSDictionaryOfVariableBindings(_dataTable)]];
    CGFloat top = 0;
    if (isIOS7) {
        top += 44;
    }
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[_dataTable(%f)]", kMainScreenHeight - PlayBarHeight - 44] options:0 metrics:0 views:NSDictionaryOfVariableBindings(_dataTable)]];
    
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.view attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_dataTable attribute:NSLayoutAttributeBottom multiplier:1 constant:PlayBarHeight]];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DownLoadInfoCell *cell=(DownLoadInfoCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentity];
...
- (void)playSong:(DownloadItem *)item
{
    if (item.downloadState == DownloadFinished) {
        ...
        song.songLocalPath = [[Utils applicationDocumentPath] stringByAppendingPathComponent:[self getLocalPathWith:item.downloadSong.songUrl]];
    ...
    [[PlayBar defultPlayer] Play:item.downloadSong];
}
