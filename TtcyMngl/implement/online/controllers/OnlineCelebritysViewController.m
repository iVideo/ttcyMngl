...
- (void)viewDidLoad
{
    ...
    UIScrollView *m_scrollView=[ImScrollView  ImScrollViewWithFrame:CGRectMake(navigationWhile+2, 0,self.mostlyView.frame.size.width-navigationWhile-4, 86) jsonUrl:nil];

    [ self.mostlyView addSubview:m_scrollView];
...
-(void)TypebtnClick:(UIButton *)selButton{
    ...
    self.Url= [NSString stringWithFormat:@"/iosSeve.ashx?type=1&&pagesize=%d&&pageCount=%d&&ctype=%d",self.pageSize,1,_nonceType];
