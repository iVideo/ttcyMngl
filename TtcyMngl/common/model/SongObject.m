...
{
    if (!([@"" isEqualToString:self.songLocalPath]||self.songLocalPath == nil || [self.songLocalPath isMemberOfClass:NSClassFromString(@"NSNull")])) {
        return [self.songUrl stringByAppendingString:@"down"];
    }
    return self.songUrl;
}
@end
