...
{
    [ShareSDK setUIStyle:SSUIStyleiOS7Below];
...
- (void)callTheShareSDKInterfaceWithSongWithDictionary:(NSDictionary *)dict
{
    ...
    NSString *imagePath = [dict objectForKey:@"avatarImageUrl"];
    NSString *contentString = @"把这首好听的歌分享给大家,点击http://www.ttcy.com获取更多歌曲";
    NSString *titleString   = @"TengrTal天堂草原蒙语歌曲";
    NSString *urlString     = [dict objectForKey:@"songUrl"];
...
- (void)callTheShareSDKInterfaceWithApp
{
    ...
    NSString *urlString     = @"http://www.ttcy.com";
    NSString *description   = @"快来用蒙文歌曲播放器听蒙语歌曲吧";
    
    id<ISSContent> publishContent = [ShareSDK content:contentString
                                       defaultContent:@"默认分享内容，没内容时显示"                                                    image:[ShareSDK imageWithPath:imagePath]
                                                title:titleString
                                                  url: urlString
                                          description:description
